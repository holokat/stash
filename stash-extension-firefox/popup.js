...
(function () {
  ...
  async function stashCurrent() {
    ...
    if (idx >= 0) {
      ...
      savedId = b.id;
      selected = new Set(bookmarkTags(b));
      noteText = b.note || '';
    } else {
      ...
      const tags = g ? [g] : [];
      const b = { id, title: (tab.title && tab.title.trim()) || domain, domain, tag: g || '', tags, note: '', url: tab.url, favIconUrl: tab.favIconUrl || '', savedAt: Date.now() };
      list.unshift(b);
      savedId = id; selected = new Set(tags); noteText = '';
    }
    await setLocal({ bookmarks: list });
    // build suggestions from the rest of the vault, then presets
    const counts = {};
    list.forEach(b => bookmarkTags(b).forEach(t => { counts[t] = (counts[t] || 0) + 1; }));
  ...
  async function persist() {
    ...
    if (!b) return;
    b.tags = uniqTags(Array.from(selected));
    b.tag = b.tags[0] || '';
    b.note = noteText;
    await setLocal({ bookmarks: list });
  }
      ...
      '<div style="position:relative;padding:22px 22px 18px;overflow:hidden;">' +
        ...
        '<div style="position:relative;display:flex;align-items:center;gap:15px;">' + logo(46) +
          '<div style="min-width:0;">' +
            ...
            '<div style="font-size:21px;font-weight:700;letter-spacing:-.01em;line-height:1.05;color:#fff;">' + titleText + (!blocked ? '<kbd class="titleShortcut" title="Quick-save shortcut">' + SAVE_SHORTCUT + '</kbd>' : '') + '</div>' +
    ...
    if (!blocked) {
      ...
      const noteCard =
        '<div style="' + card + 'padding:16px;margin:0 16px 16px;">' +
          ...
          '<textarea data-focus-id="note" placeholder="Add a quick thought, context, or why this matters." style="width:100%;min-height:72px;font-size:13.5px;line-height:1.5;color:#e8e8ec;">' + esc(noteText) + '</textarea>' +
        '</div>';
    ...
    const footer =
      '<div style="display:flex;align-items:center;gap:9px;padding:4px 16px 18px;">' +
        (blocked ? '' :
          ...
          '</div>') +
        '<div class="btn btnG vsi" data-act="vault" style="' + ghost + '">Open Vault <kbd class="btnKbd">' + VAULT_SHORTCUT + '</kbd></div>' +
  ...
  function addTag(raw) {
    const t = cleanTag(String(raw || '').replace(/[, ]+$/, ''));
    if (!t) return;
  ...
  async function boot() {
    ...
    if (!blocked) { try { await stashCurrent(); } catch (e) { blocked = true; } }
      ...
      const { bookmarks } = await getLocal('bookmarks');
      const counts = {};
      (bookmarks || []).forEach(b => bookmarkTags(b).forEach(x => counts[x] = (counts[x] || 0) + 1));
