...
(function () {
  ...
  function cleanTag(t) { return String(t || '').trim().toLowerCase(); }
  function uniqTags(tags) {
    const out = [];
    (tags || []).forEach(t => { const clean = cleanTag(t); if (clean && out.indexOf(clean) < 0) out.push(clean); });
    return out;
  }
  function bookmarkTags(b) {
    return uniqTags(Array.isArray(b && b.tags) ? b.tags : (b && b.tag ? [b.tag] : []));
  }
  function primaryTag(b) {
    const tags = bookmarkTags(b);
    return tags[0] || '';
  }
  function normalizeBookmark(b) {
    const tags = bookmarkTags(b);
    return Object.assign({}, b, { tags, tag: tags[0] || '' });
  }
  ...
  function demoSeed() {
    ...
    ].map((d, i) => ({ id: i + 1, title: d[0], domain: d[1], tag: d[2], tags: [d[2]], note: d[3], url: 'https://' + d[1], favIconUrl: '', savedAt: now - d[4] * day }));
  }
  ...
  function persist() { state.bookmarks = state.bookmarks.map(normalizeBookmark); store.set({ bookmarks: state.bookmarks }); }
  ...
  function saveDraft() {
    ...
    const tags = uniqTags(state.draftTags || []);
    const id = state.bookmarks.reduce((m, b) => Math.max(m, b.id || 0), 0) + 1;
    state.bookmarks.unshift(normalizeBookmark({ id, title, domain, tag: tags[0] || '', tags, note: '', url, favIconUrl: '', savedAt: Date.now() }));
  ...
  function tagChipsHTML(tags, style, limit) {
    const shown = tags.slice(0, limit);
    let html = shown.map(t => '<span style="' + style + '">' + esc(t) + '</span>').join('');
    if (tags.length > limit) html += '<span style="' + style + '">+' + (tags.length - limit) + '</span>';
    return html;
  }
  ...
  function vm(b, th, isList) {
    ...
    const actBtn = 'width:34px;height:34px;border-radius:50%;background:' + th.actBg + ';color:' + th.actIcon + ';display:flex;align-items:center;justify-content:center;cursor:pointer;';
    const fav = favicon(b);
    return {
      id: b.id, title: b.title, domain: b.domain, tag: tags[0] || '', tags, tagText: tags.join(' · '), note: b.note || '', ageLabel: age(daysAgoOf(b)),
    ...
    return '<div class="vcard" style="' + c.style + '">' +
      '<div style="padding:16px;">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">' +
          ...
          '<div style="display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap;max-width:72%;">' + tagChipsHTML(c.tags, c.chipStyle, 2) + '</div>' +
        '</div>' +
      ...
      '</div>' +
      '<div class="vact" style="position:absolute;top:11px;right:11px;display:flex;gap:8px;opacity:0;transform:translateY(-6px);transition:opacity .35s,transform .35s;">' +
  ...
  function rowHTML(c) {
    return '<div class="vrow" style="' + c.rowStyle + '">' +
      ...
      '</div>' +
      '<div style="display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap;max-width:260px;">' + tagChipsHTML(c.tags, c.chipStyle, 3) + '</div>' +
      ...
      '<div class="vact" style="display:flex;gap:8px;opacity:0;transform:translateX(6px);transition:opacity .3s,transform .3s;margin-left:6px;">' +
        '<div class="vactBtn" data-act="open" data-id="' + c.id + '" style="' + c.actBtnStyle + '">' + ICON_OPEN + '</div>' +
        '<div class="vactBtn" data-act="del" data-id="' + c.id + '" style="' + c.actBtnStyle + '">' + ICON_DEL + '</div>' +
      '</div>' +
    '</div>';
    ...
    return '<div class="vfeat" style="' + f.style + '">' +
      ...
      '<div style="position:absolute;left:28px;right:28px;bottom:24px;display:flex;align-items:flex-end;justify-content:space-between;gap:20px;">' +
        '<div style="min-width:0;">' +
          '<div style="font-family:\'JetBrains Mono\',monospace;font-size:12px;color:rgba(255,255,255,.72);margin-bottom:10px;letter-spacing:.02em;">' + esc(f.domain) + (f.tagText ? ' · ' + esc(f.tagText) : '') + '</div>' +
      ...
      '</div>' +
      '<div class="vfeatAct" style="position:absolute;top:16px;right:16px;display:flex;gap:8px;opacity:0;transform:translateY(-6px);transition:opacity .35s,transform .35s;">' +
        '<div data-act="del" data-id="' + f.id + '" class="vactBtn" style="width:34px;height:34px;border-radius:50%;background:rgba(0,0,0,.5);color:#fff;border:1px solid rgba(255,255,255,.18);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2m2 0-.7 12a2 2 0 0 1-2 1.9H8.7a2 2 0 0 1-2-1.9L6 7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></div>' +
      '</div>' +
    '</div>';
  }
  ...
  function pill(label, on, dataAttrs, th) {
    const style = on
      ? 'font-size:12.5px;font-weight:600;color:var(--on);background:linear-gradient(135deg,var(--ac2),var(--ac));border-radius:999px;padding:6px 13px;cursor:pointer;white-space:nowrap;'
      : 'font-size:12.5px;color:' + th.tagText + ';background:' + th.tagBg + ';border-radius:999px;padding:6px 13px;cursor:pointer;white-space:nowrap;';
    return '<span class="vtag" ' + dataAttrs + ' style="' + style + '">' + esc(label) + '</span>';
  }

  function seg(on, th, dark) {
    return 'width:38px;height:34px;border-radius:9px;display:flex;align-items:center;justify-content:center;cursor:pointer;' +
  ...
  function buildHTML() {
    const th = theme();
    const dark = th.dark;
    const vars = '--ac:' + ACCENT + ';--ac2:' + ACCENT2 + ';--on:' + ON_ACCENT + ';--acSoft:' + rgba(ACCENT, 0.15) + ';--acB:' + rgba(ACCENT, 0.30) + ';--acSh:' + rgba(ACCENT, 0.32) + ';--acGlow:' + rgba(ACCENT, 0.55) + ';--acDim:' + rgba(ACCENT, 0.10) + ';--wash:' + rgba(ACCENT, dark ? 0.20 : 0.12) + ';--rowHover:' + th.rowHover + ';--shortcutBg:' + (dark ? 'rgba(255,255,255,.045)' : '#fff') + ';--shortcutBorder:' + th.chromeBorder + ';--shortcutText:' + th.sub + ';--shortcutStrong:' + th.title + ';--shortcutKeyBg:' + (dark ? 'rgba(255,255,255,.055)' : '#F6F4EE') + ';--shortcutKeyBorder:' + th.chromeBorder + ';--shortcutKeyText:' + ACCENT2 + ';';

    document.body.style.background = th.pageBg;

    // tags
    const counts = {};
    state.bookmarks.forEach(b => { bookmarkTags(b).forEach(t => { counts[t] = (counts[t] || 0) + 1; }); });
    ...
    const cmp = {
      ...
      tag: (a, b) => primaryTag(a).localeCompare(primaryTag(b)) || a.title.localeCompare(b.title)
    };
    ...
    if (list.length === 0) {
      let emptyTitle = 'Nothing here yet', emptySub = 'Saved links will appear in this vault. Press the + button or ' + SAVE_SHORTCUT + ' on any tab.';
          ...
          '<div style="display:flex;align-items:center;gap:10px;flex:1 1 260px;justify-content:flex-end;min-width:220px;">' +
            '<div style="display:flex;align-items:center;gap:9px;background:' + th.chromeBg + ';border:1px solid ' + th.chromeBorder + ';border-radius:999px;padding:10px 16px;flex:1;min-width:0;max-width:380px;">' +
              ...
              (state.query
                ...
                : '<kbd style="font-family:\'JetBrains Mono\',monospace;font-size:10.5px;color:' + th.iconMuted + ';background:' + (dark ? 'rgba(255,255,255,.06)' : '#fff') + ';border:1px solid ' + th.chromeBorder + ';border-radius:6px;padding:3px 6px;line-height:1;white-space:nowrap;flex:none;">' + SEARCH_SHORTCUT + '</kbd>') +
            '</div>' +
            '<div class="vbig" data-act="add-open" style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,var(--ac2),var(--ac));color:var(--on);display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px -4px var(--acGlow);cursor:pointer;"><svg width="17" height="17" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="var(--on)" stroke-width="2.4" stroke-linecap="round"/></svg></div>' +
          '</div>' +
        '</div>' +
        '<div style="display:flex;gap:8px;align-items:center;padding-bottom:16px;flex-wrap:wrap;">' + tagsHTML + '</div>' +
        '<div class="vshortcutBar" aria-label="Keyboard shortcuts">' +
          '<span class="vshortcutHint"><strong>Quick save tab</strong><kbd>' + SAVE_SHORTCUT + '</kbd></span>' +
          '<span class="vshortcutHint"><strong>Open vault</strong><kbd>' + VAULT_SHORTCUT + '</kbd></span>' +
          '<span class="vshortcutHint"><strong>Search</strong><kbd>' + SEARCH_SHORTCUT + '</kbd></span>' +
        '</div>' +
    ...
    if (state.addOpen) {
      ...
      const saveBtnStyle = canSave
        ? 'padding:11px 20px;border-radius:11px;background:linear-gradient(135deg,var(--ac2),var(--ac));color:var(--on);font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 6px 18px -6px var(--acGlow);'
  ...
  function setupDelegation() {
    app.addEventListener('click', (e) => {
      ...
      switch (act) {
        ...
        case 'add-open': setState({ addOpen: true, draftUrl: '', draftTitle: '', draftTags: [] }); break;
        case 'add-close': setState({ addOpen: false }); break;
        case 'overlay-close': if (e.target === t) setState({ addOpen: false }); break;
        case 'modal-tag': { const tag = cleanTag(k); const arr = uniqTags(state.draftTags || []); if (tag && arr.indexOf(tag) < 0) arr.push(tag); setState({ draftTags: arr }); break; }
        case 'modal-untag': { const tag = cleanTag(k); setState({ draftTags: uniqTags(state.draftTags || []).filter(x => x !== tag) }); break; }
    ...
    app.addEventListener('keydown', (e) => {
      ...
      if (id === 'modaltag' && (e.key === 'Enter' || e.key === ',')) { e.preventDefault(); const v = cleanTag(String(e.target.value || '').replace(/[, ]+$/, '')); if (v) { const arr = uniqTags(state.draftTags || []); if (arr.indexOf(v) < 0) arr.push(v); setState({ draftTags: arr }); } }
